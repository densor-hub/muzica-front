...
import ProfileIconAndMenu from "../components/Private-Sites/Modals/profile-icon-and-menu";
...
const PrivateRoute =()=>{
    ...
    if(auth?.stagenameInUrl){ 
        if(window.location.href.indexOf(auth.stagenameInUrl)>0 || window?.location?.href?.includes(auth?.stagenameInUrl)){
